...
class Antrian extends Controller
{
    ...
    public function add()
    {
        ...
        $tgl = date('y-m-d');
            ...
            echo '<script>
                    alert("Sukses Tambah Data Antrian");
    ...
    public function rekdis($id)
    {
        ...
        {
            $data['antrian'] = $getAntrian;
            $data['title']  = 'Catatan Dokter '.$getAntrian->id;
            echo view('dokter_header_view', $data);
            echo view('rekdis_antrian_view', $data);
            echo view('footer_view', $data);
            ...
            echo '<script>
                    alert("ID Antrian '.$id.' Tidak ditemukan");
                    window.location="'.base_url('DHistory').'"
                </script>';
        }
    }
    ...
    public function update2()
    {
        ...
        $data = array(
            'CATDOK' => $this->request->getPost('catdok')
        );
        $model->editRekdis($data,$id);
        echo '<script>
                alert("Sukses Tambah Catatan Dokter");
                window.location="'.base_url('DHistory').'"
            </script>';
    ...
    public function update($id)
    {
        ...
        if(isset($getAntrian))
        {
            ...
            echo '<script>
                alert("Data Antrian Sudah Update");
                window.location="'.base_url('antrian').'"
            </script>';
        }else{
            echo '<script>
